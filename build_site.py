...
讀取 index/master-index.json，產出 docs/index.html
"""
...
def build_site():
    ...
    html = _generate_html(articles, by_date, dates, total, regions, categories, latest_date)
    (docs_dir / "index.html").write_text(html, encoding="utf-8")
    print(f"Site built: docs/index.html ({total} articles, {len(dates)} dates)")

    # CNAME (optional, remove if not using custom domain)
    # (docs_dir / "CNAME").write_text("insurance.example.com")
...
def _generate_html(articles, by_date, dates, total, regions, categories, latest_date):
    ...
    for date in dates:
        ...
        cards_html += f'<h2 class="date-header">{date} ({len(date_articles)} 篇)</h2>\n'
        ...
        for a in date_articles:
            ...
            kw_html = " ".join(f'<span class="tag">{k}</span>' for k in keywords[:4])
            companies = a.get("companies", [])
            co_html = " ".join(f'<span class="company-tag">{c}</span>' for c in companies[:3]) if companies else ""

            cards_html += f'''<div class="card" data-region="{region}" data-category="{category}" data-importance="{importance}">
  <div class="card-header">
    <span class="region-badge" style="background:{color}">{region}</span>
    <span class="importance">{icon}</span>
    <span class="category">{cat_icon} {category}</span>
  </div>
...
.card {{ background:white; border-radius:12px; padding:16px; box-shadow:0 2px 8px rgba(0,0,0,0.08); transition:transform 0.2s, box-shadow 0.2s; border-left:4px solid #e0e0e0; }}
...
.card-title {{ font-size:15px; line-height:1.4; margin-bottom:6px; }}
.card-title a {{ color:#2c3e50; text-decoration:none; }}
.card-title a:hover {{ color:#3498db; }}
.card-summary {{ font-size:13px; color:#666; line-height:1.5; margin-bottom:8px; }}
...
.card-companies {{ display:flex; gap:4px; flex-wrap:wrap; margin-top:4px; }}
